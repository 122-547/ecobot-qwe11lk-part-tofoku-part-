...
async def equip_role(inter: disnake.Interaction, settings: Settings, role_id: int) -> Optional[bool]:
    try:
        if not role_id:
        ...
        slots = slots[0][1:]
        slot_num = slots.index(None)+1
        query = f"UPDATE equipped SET [{str(slot_num)}]=? WHERE id=?"
        await conn.execute(query, (role_id, inter.author.id))
        ...
        logger.error(f"Equip role function error: {e}")
        return
    finally:
